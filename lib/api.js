...
const generateMarkdown = (changes, version) => {
  ...
  let content = `
<h2 class="ReleaseVersion">${version}</h2>\n
<p class="ReleaseDate">
  <time datetime="${today.toISOString()}">${timeStamp}</time>
</p>\n
`
...
/**
 * Bump the version in the nearest package.json (and lock file, if it exists).
 *
 * @async
 *
 * @param {string} version New version string.
 ...
 */
const bumpPackageFiles = async (version) => {
  getCurrentVersion()
  ...
  const lockPath = __pkg.path.replace('package.json', 'package-lock.json')
  if (await fs.pathExists(lockPath)) {
    const newLockContent = { ...require(lockPath) }
    // Update top-level version value
    newLockContent.version = version
    // Update root package version value
    if (newLockContent.packages['']) {
      newLockContent.packages[''].version = version
    }

    await fs.writeJson(lockPath, newLockContent, { spaces: 2 })
  }
...
const generateLogYml = async (fileName, config) => {
  ...
  await fs.copy(__dirname + '/template.yml', config.source + '/' + purifiedName)
